...
class GInputFile {
  ...
  public:
    ...
    GInputFile(const char* nom){ fileName = std::string(nom) ;  }
	GInputFile(const std::string& nom){ fileName = nom ;  }
	...
	bool getNextLine(ifstream &_file, const char* separators, vector<string>* value);
    char* getNextToken() ;
